...
parser.add_argument("--restore_path", type=str, default="/home/eslam/Masters/NN/IWild_project/YOLOv3_TensorFlow/checkpoint/model-epoch_9_step_53739_loss_5.0000_lr_9.6e-05",
...
with tf.Session() as sess:
    ...
    for j in trange(args.img_cnt):
        ...
        if len(pred_content) != 0:
            if len(pred_content) > 1:
                max_score = pred_content[0][5]
                max_index = 0
                for k in range(len(pred_content)):
                    if pred_content[k][5] >= max_score:
                        max_index = k
                f.write("%d %d \r\n" % (__image_ids, pred_content[max_index][6]+1))
            else:
                f.write("%d %d \r\n" % (__image_ids, pred_content[0][6]+1))
        else:
            f.write("%d %d \r\n" % (__image_ids, 0))
