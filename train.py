...
if not args.save_optimizer:
    saver_to_save = tf.train.Saver(max_to_keep=100)
    saver_best = tf.train.Saver(max_to_keep=100)
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        if epoch % args.save_epoch == 0 and epoch > 0:
            #if tmp_total_loss <= 2.:
                saver_to_save.save(sess, args.save_dir + 'model-epoch_{}_step_{}_loss_{:.4f}_lr_{:.5g}'.format(epoch, int(__global_step), 5, __lr))
